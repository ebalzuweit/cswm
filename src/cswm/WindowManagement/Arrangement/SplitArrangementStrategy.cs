...
public class SplitArrangementStrategy : IArrangementStrategy
{
	public IEnumerable<WindowLayout> Arrange(IEnumerable<MonitorLayout> monitorLayouts)
	{
		return monitorLayouts.SelectMany(monitorLayout =>
			PartitionSpace(monitorLayout.Space, monitorLayout.Windows, splitRatio: monitorLayout.AspectRatio));
	}

	private IEnumerable<WindowLayout> PartitionSpace(Rect space, IEnumerable<WindowLayout> windows, float splitRatio = 1.7777777778f)
	{
		if (windows.Count() == 0)
		...
		{
			return new[] {
				new WindowLayout(windows.First().hWnd, space)
			};
		}
